...
def package(request):
    ...
    if request.method=="POST":
        ...
        tblbooking(username=username,email=email,packagename=package,pprice=price,totalprice=totalprice,npeople=npeople,no_of_days=ndays,bookingdatefor=bdate,time=btime,booking_date=datetime.now().date()).save()
...
def myblogs(request):
    ...
    if request.method=="POST":
        ...
        tblblogs(name=name,email=email,user_pic=userpic,title=title,description=info,city=city,picture=picture,posteddate=datetime.now().date()).save()
        d={"success":True}
    d["blogs"]=bdata
...
def profile(request):
    ...
    if request.method=="POST":
        ...
        return HttpResponse("<script>alert('your profile has updated');location.href='/blogger/profile/';</script>")
    d={"pdata":data}
